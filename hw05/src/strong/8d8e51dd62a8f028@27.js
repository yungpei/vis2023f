...
md`### 利用蘋果成績圖環繞個人照片(小組) (2pt)`
)}

function _simple(d3,data,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1500;
  const height = 1000;
...
FileAttachment("group03.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["group03.json", {url: new URL("../group03.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer()).define(["md"], _2);
  main.variable(observer("simple")).define("simple", ["d3","data","drag","url_list","invalidation"], _simple);

...
md`<div style="color: grey; font: 13px/25.5px var(--sans-serif); text-transform: uppercase;"><h1 style="display: none;">HW01 Strong baseline</h1><a href="https://d3js.org/">D3</a> › <a href="/@d3/gallery">Gallery</a></div>

# HW01 Strong baseline (4pt)

繪製 v7 創新成績表，參考 D3畫廊，修改範例並推出創新成績表。`
)}
...
function _simple1(d3,simple,drag,url_list,invalidation)
{
  ...
  const simulation = d3.forceSimulation(nodes)
      ...
      //防止節點重疊
      .force('collide',d3.forceCollide().radius(30).iterations(0))
      .force("x", d3.forceX())
      .force("y", d3.forceY());
  ...
  for (let i = 0; i < numImages; i++) {
      const angle = (i / numImages) * 2 * Math.PI; // 計算每個圖片的角度
      const x = imageRadius * Math.cos(angle); // 計算 x 座標
      const y = imageRadius * Math.sin(angle); // 計算 y 座標
      imagesGroup.append("image")
          .attr("xlink:href",
          d => {
            if(d.data.leval == 2){ // 如果有子節點，顯示子節點的資訊
               const score = parseInt(d.data[`Hw${i+1}_score`]);
                return url_list[score]
            }
          }) // 設定圖片連結，這裡假設每個節點有一個 imageURL 屬性
  ...
  node.append("title")
        ...
        /*else if(d.data.leval == 2){
          ...
          return  "序號 : "+d.data.Group+
                  "\n系所 : "+d.data.Department+
                  "\n學號 : "+d.data.Classnumber+
                  "\n姓名 : "+d.data.Name+
                  "\nGitHub 帳號 : "+d.data.GitHub+
...
FileAttachment("simple.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["simple.json", {url: new URL("./simple.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
